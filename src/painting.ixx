...
static int paint_timers(HDC hdc, int cw, int y, PaintCtx& ctx, sc::time_point now) {
    ...
    for (int i = 0; i < (int)ctx.app.timers.size(); ++i) {
        ...
        std::wstring tstr =
            touched ? format_timer_display(ts.t.remaining(now)) : format_timer_edit(duration_cast<Timer::dur>(ts.dur));
        COLORREF tcol = expired ? th.expire : (touched && ts.t.remaining(now) < 10s) ? th.warn : th.text;
        if (touched) {
            SelectObject(hdc, ctx.fontSm);
            SetTextColor(hdc, th.dim);
        ...
        if ((int)ctx.app.timers.size() > 1)
            btn(hdc, {pm_margin, pm_top, pm_margin + pm_sz, pm_top + pm_sz}, false, L"\u2212", tmr_act(i, A_TMR_DEL),
                ctx);
