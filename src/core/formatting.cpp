...
std::string format_stopwatch_short(std::chrono::steady_clock::duration d) {
    ...
    auto s = total_s % 60;
    auto m = total_s / 60;
    return std::format("{:02}:{:02}.{:03}", m, s, ms);
}

std::string format_stopwatch_long(std::chrono::steady_clock::duration d) {
    auto total_ms = std::max(std::chrono::milliseconds::rep{0}, std::chrono::duration_cast<std::chrono::milliseconds>(d).count());
    auto ms = total_ms % 1000;
    auto total_s = total_ms / 1000;
    auto s = total_s % 60;
...
std::string format_stopwatch_display(std::chrono::steady_clock::duration d) {
...
std::string format_lap_row(std::size_t lap_number, std::chrono::steady_clock::duration split,
                                   std::chrono::steady_clock::duration total) {
    return std::format("Lap {:<3}   split {:<14}   total {}", lap_number, format_stopwatch_short(split),
