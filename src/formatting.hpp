...
inline std::wstring format_stopwatch_short(steady_duration d) {
...
inline std::wstring format_stopwatch_long(steady_duration d) {
...
inline std::wstring format_stopwatch_display(steady_duration d) {
    return d >= std::chrono::hours{1} ? format_stopwatch_long(d) : format_stopwatch_short(d);
}

inline std::wstring format_timer_display(steady_duration d) {
    auto total_s = std::chrono::duration_cast<std::chrono::seconds>(d).count();
    if (total_s >= 3600) {
        auto h = total_s / 3600;
        auto m = (total_s / 60) % 60;
...
inline std::wstring format_timer_edit(steady_duration d) {
    auto total_s = std::chrono::duration_cast<std::chrono::seconds>(d).count();
    auto h = total_s / 3600;
    auto m = (total_s / 60) % 60;
    auto s = total_s % 60;
...
inline std::wstring format_lap_row(std::size_t lap_number, steady_duration split, steady_duration total) {
