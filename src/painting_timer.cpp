...
// ─── Timer painters ───────────────────────────────────────────────────────
void paint_timer_progress(HDC hdc, int cw, int y, int i, std::chrono::steady_clock::time_point now, PaintCtx& ctx) {
...
void paint_timer_running(HDC hdc, int cw, int y, int i, std::chrono::steady_clock::time_point now, PaintCtx& ctx) {
    CHRONOS_ASSERT(i >= 0 && i < (int)ctx.app.timers.size());
    auto& layout = ctx.layout;
    auto& th = ctx.theme;
    auto& ts = ctx.app.timers[i];
...
void paint_timer_controls(HDC hdc, int cw, int y, int i, PaintCtx& ctx) {
    CHRONOS_ASSERT(i >= 0 && i < (int)ctx.app.timers.size());
    auto& layout = ctx.layout;
    auto& ts = ctx.app.timers[i];
